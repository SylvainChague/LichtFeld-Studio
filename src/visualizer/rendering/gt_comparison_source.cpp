...
namespace lfs::vis {
    ...
    void RenderingManager::setGTComparisonActualSizeError(std::string error) {
        if (gt_comparison_actual_size_state_.error != error) {
            gt_comparison_actual_size_state_.error = std::move(error);
    ...
    void RenderingManager::invalidateGTComparisonImageCache() {
        ...
        setGTComparisonActualSizeError({});
    ...
    std::shared_ptr<lfs::core::Tensor> RenderingManager::ensureCudaGTViewportImage(
        std::shared_ptr<lfs::core::Tensor> image,
    ...
    void RenderingManager::invalidateGTComparisonActualSizeResources(const bool preserve_ready_source) {
        std::optional<GTComparisonFullSourceSlot> released_full_source;
        {
            std::lock_guard lock(gt_comparison_image_mutex_);
    ...
    void RenderingManager::retryGTComparisonActualSize() {
        ...
        auto& state = gt_comparison_actual_size_state_;
    ...
    RenderingManager::prepareGTActualFrame(
        ...
        const auto fail_tile = [&](std::string error) {
            ...
            setGTComparisonActualSizeError(frame.error);
        ...
        if (gt_comparison_actual_size_state_.source_key != source_key) {
            const auto requested_at = gt_comparison_actual_size_state_.requested_at;
            setGTComparisonActualSizeError({});
            gt_comparison_actual_size_state_.reset();
            gt_comparison_actual_size_state_.source_key = source_key;
            gt_comparison_actual_size_state_.requested_at =
        ...
        if (lookup.status == GTComparisonImageStatus::Failed) {
            setGTComparisonActualSizeError(lookup.error);
        }
        ...
        try {
            ...
            const lfs::core::CUDAStreamGuard guard(stream);
            auto cuda_tile = ensureCudaGTViewportImage(
                gt_comparison_actual_size_state_.visible_tile,
                camera.uid(),
                crop.extent,
